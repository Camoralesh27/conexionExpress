...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function getErrorEl(field){
    return field.closest('.contact__field').querySelector('.contact__error');
  }
  ...
  function validateField(field){
    ...
    /* NOMBRE */
    if (name === 'name'){
      if (isBlankIgnoringSpaces(value)){
        showError(field, 'Por favor, ingresa tu nombre completo.');
        return false;
      }
      clearError(field);
      return true;
    ...
    /* TELÉFONO */
    if (name === 'phone'){
      const digitsOnly = value.replace(/\D/g, ''); // solo números

      if (isBlankIgnoringSpaces(value)){
        showError(field, 'Por favor, ingresa tu número de teléfono.');
        return false;
      }
      ...
      if (digitsOnly.length > 15){
        showError(field, 'El teléfono no parece válido. Revisa que esté bien escrito.');
        return false;
      }
    ...
    if (name === 'email'){
      if (isBlankIgnoringSpaces(value)){
        showError(field, 'Por favor, ingresa tu correo electrónico.');
        return false;
      }
  ...
  async function sendWithWeb3Forms(){
    ...
    return await res.json(); // { success: true/false, message: "" }
  }
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      submitBtn.disabled = true; // se habilita cuando el usuario vuelve a escribir
    }
  });
